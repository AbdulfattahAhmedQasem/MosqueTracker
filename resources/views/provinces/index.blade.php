...
<head>
    ...
    <title>إدارة المحافظات - نظام إدارة المساجد</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-4xl font-bold text-slate-800 mb-2">إدارة المحافظات</h1>
            </div>
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">عدد الأحياء</th>
                    ...
                    <tbody>
                        @forelse($provinces as $province)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                <td class="px-4 py-4 text-slate-700 font-medium">{{ $province->name }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $province->neighborhoods_count ?? 0 }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $province->mosques_count ?? 0 }}</td>
                                <td class="px-4 py-4">
                                    <div class="flex gap-1">
                                        ...
                                        @can('delete provinces')
                                            <form action="{{ route('provinces.destroy', $province) }}" method="POST" class="inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-slate-500">لا توجد محافظات مسجلة</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
