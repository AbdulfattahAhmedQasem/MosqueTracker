...
<head>
    ...
    <title>تفاصيل المحافظة - {{ $province->name }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 max-w-6xl mx-auto">
        <div class="mb-6">
            <h1 class="text-4xl font-bold text-slate-800 mb-2">تفاصيل المحافظة</h1>
        </div>
        ...
        <div class="bg-slate-50 rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-slate-600 mb-1">اسم المحافظة</p>
                    <p class="font-semibold text-slate-800">{{ $province->name }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-600 mb-1">عدد الأحياء</p>
                    <p class="font-semibold text-slate-800">{{ $province->neighborhoods->count() }}</p>
                </div>
            </div>
        </div>

        <!-- قائمة الأحياء -->
        @if($province->neighborhoods->count() > 0)
        <div class="bg-white border border-slate-200 rounded-lg p-6 mb-6">
            <h2 class="text-2xl font-bold text-slate-800 mb-4">الأحياء</h2>
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">اسم الحي</th>
                    ...
                    <tbody>
                        @foreach($province->neighborhoods as $neighborhood)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                <td class="px-4 py-4 text-slate-700 font-medium">{{ $neighborhood->name }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $neighborhood->mosques->count() }}</td>
                                <td class="px-4 py-4">
