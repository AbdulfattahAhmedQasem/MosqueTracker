...
<head>
    ...
    <title>تعديل بيانات المحافظة - {{ $province->name }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-4xl font-bold text-slate-800 mb-2">تعديل بيانات المحافظة</h1>
            <a href="{{ route('provinces.show', $province) }}" class="text-blue-500 hover:text-blue-600 flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
                العودة لتفاصيل المحافظة
            </a>
        </div>
        ...
        <form action="{{ route('provinces.update', $province) }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">اسم المحافظة *</label>
                    <input
                        type="text"
                        name="name"
