...
<head>
    ...
    <title>إدارة السكن - نظام إدارة المساجد</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-4xl font-bold text-slate-800 mb-2">إدارة السكن</h1>
            </div>
            @can('create housings')
            <a href="{{ route('housing.create') }}"
                    ...
                    <tbody>
                        @forelse($housings as $housing)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-4 py-4 text-slate-700">{{ $housing->mosque->name ?? '-' }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $housing->member->name ?? '-' }}</td>
                                <td class="px-4 py-4">
                                    <div class="flex gap-1">
                                        ...
                                        @endcan
                                        @can('delete housings')
                                        <form action="{{ route('housing.destroy', $housing) }}" method="POST" class="inline"
                                              onsubmit="return confirm('هل أنت متأكد من حذف هذا السكن؟')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-slate-500">لا يوجد سكن مسجل</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
