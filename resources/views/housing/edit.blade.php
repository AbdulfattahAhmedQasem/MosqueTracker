...
<head>
    ...
    <title>تعديل بيانات السكن - {{ $housing->name }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-4xl font-bold text-slate-800 mb-2">تعديل بيانات السكن</h1>
            <a href="{{ route('housing.show', $housing) }}" class="text-blue-500 hover:text-blue-600 flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
                العودة لتفاصيل السكن
            </a>
        </div>
        ...
        <form action="{{ route('housing.update', $housing) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($mosques as $mosque)
                            <option value="{{ $mosque->id }}" {{ old('mosque_id', $housing->mosque_id) == $mosque->id ? 'selected' : '' }}>
