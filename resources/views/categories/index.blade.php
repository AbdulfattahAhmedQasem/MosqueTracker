...
<head>
    ...
    <title>إدارة الفئات - نظام إدارة المساجد</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-4xl font-bold text-slate-800 mb-2">إدارة الفئات</h1>
            </div>
                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                <td class="px-4 py-4 text-slate-700 font-medium">{{ $category->name }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $category->description ?? '-' }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $category->members_count ?? 0 }}</td>
                                <td class="px-4 py-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-slate-500">لا توجد فئات مسجلة</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
