...
<head>
    ...
    <title>تفاصيل المهنة - {{ $profession->name }}</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 max-w-6xl mx-auto">
        <div class="mb-6">
            <h1 class="text-4xl font-bold text-slate-800 mb-2">تفاصيل المهنة</h1>
        </div>
        ...
        <div class="bg-slate-50 rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-slate-600 mb-1">اسم المهنة</p>
                    <p class="font-semibold text-slate-800">{{ $profession->name }}</p>
                </div>
                <div>
                    <p class="text-sm text-slate-600 mb-1">عدد المنسوبين</p>
                    <p class="font-semibold text-slate-800">{{ $profession->members->count() }}</p>
                </div>
                @if($profession->description)
                <div class="col-span-2">
                    <p class="text-sm text-slate-600 mb-1">الوصف</p>
                    <p class="font-semibold text-slate-800">{{ $profession->description }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-lg p-6 mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach($profession->members as $member)
