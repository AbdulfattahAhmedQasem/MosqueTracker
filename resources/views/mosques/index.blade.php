...
<head>
    ...
    <title>إدارة المساجد - نظام إدارة المساجد</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-4xl font-bold text-slate-800 mb-2">إدارة المساجد والجوامع</h1>
            </div>
            @can('create mosques')
                ...
                <table class="w-full">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">النوع</th>
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">الحي</th>
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">المحافظة</th>
                            <th class="px-4 py-4 text-right text-sm font-semibold text-slate-700">عدد السكن</th>
                    ...
                    <tbody>
                        @forelse($mosques as $mosque)
                            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                <td class="px-4 py-4 text-slate-700 font-medium">{{ $mosque->name }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $mosque->type }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $mosque->neighborhood->name ?? '-' }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $mosque->neighborhood->province->name ?? '-' }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $mosque->housings_count ?? 0 }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $mosque->members_count ?? 0 }}</td>
                                <td class="px-4 py-4">
                                    <div class="flex gap-1">
                                        <a href="{{ route('mosques.show', $mosque) }}"
                                        ...
                                        @endcan
                                        @can('delete mosques')
                                        <form action="{{ route('mosques.destroy', $mosque) }}" method="POST" class="inline"
                                              onsubmit="return confirm('هل أنت متأكد من حذف هذا المسجد؟')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-8 text-center text-slate-500">لا توجد مساجد مسجلة</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
